...
public class MessagesListAdapter<MESSAGE extends IMessage>
    ...
    public void addToEnd(List<MESSAGE> messages, boolean reverse) {
        ...
        if (!items.isEmpty()) {
            ...
            Wrapper lastWrapper = items.get(lastItemPosition);
            if (lastWrapper.item instanceof Date &&
                    DateFormatter.isSameDay(messages.get(0).getCreatedAt(), (Date) lastWrapper.item)) {
        ...
        for (MESSAGE message : messages) {
            // 날짜 헤더도 함께 추가하고 싶다면 여기에 조건 처리
            items.add(new Wrapper<>(message));
        }

        Log.d("MessagesListAdapter", "✅ items.size after add = " + items.size());
    ...
    /* public void upsert(MESSAGE message) {
        if (!update(message)) {
            addToEnd(Collections.singletonList(message), false);
        }
    }
